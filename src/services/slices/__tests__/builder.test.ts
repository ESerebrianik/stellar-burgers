...
describe('Проверка slices builder', () => {
  ...
  test('Проверка изменения порядка ингредиентов', () => {
    const reorderTestState: TConstructorState = {
      bun: null,
      ingredients: [
        {
    ...
    const newState = builderReducer(reorderTestState, action);
